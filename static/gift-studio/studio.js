...
async function createProject(name){if(state.dirty&&!confirm('Discard unsaved changes and create a project?'))return;const body=await request('/projects',{method:'POST',body:JSON.stringify({name})});state.projects=await request('/projects').then(x=>x.projects||[]);renderProjectSelect();setProject(body.project);}
async function deleteCurrentProject(){if(!state.project)return;const name=state.project.name||state.project.id;if(!confirm(`Delete project “${name}”? This cannot be undone.`))return;await request(`/projects/${encodeURIComponent(state.project.id)}`,{method:'DELETE'});state.projects=await request('/projects').then(x=>x.projects||[]);if(state.projects.length)await loadProject(state.projects[0].id);else await createProject('My Gift Cards');toast(`Deleted ${name}.`,'success');}
async function loadProject(id){if(!id)return;if(state.dirty){if(!confirm('Discard unsaved changes and switch projects?')){renderProjectSelect();return;}}const body=await request(`/projects/${encodeURIComponent(id)}`);setProject(body.project);}
